...
#entradas:
s1 = 19 #presencia
s2 = 21 #PET proveniente de arduino
s3 = 22 #obstrucciones 
...
class advertencia(QMainWindow):
    ...
    def back(self):
        ...
        if((GPIO.input(s3)) == (GPIO.input(s4)) == 0):
          self.hide()
...
class principal(QMainWindow):
    ...
    def condiciones(self):
       #deteccion de obstrucciones y/o temperatura y humedad
        if((GPIO.input(s3)) or (GPIO.input(s4)) == 1):
            self.hide()
    ...
    def medir(self):
        ...
        principal.distancia_vacio=50 #x1  100%=y1
        principal.distancia_lleno=6 #x2 0%=y2
        
        #interpolacionlineal= y=y1+((y2-y1)/(x2-x1))*(x-x1)
        nivel=(100+((0-100)/(6-50))*(principal.dist-50))
...
class ingresopet(QMainWindow):
    ...
    def cont(self):
        ...
        if (GPIO.input(s1) == GPIO.input(s2) == 1 ) and (GPIO.input(s5) == 0):
              self.hide()    
...
class aceptada(QMainWindow):
    ...
    #funcion continuar donde se evalua que el contenedor no este lleno
    def cont(self):
        self.hide()
